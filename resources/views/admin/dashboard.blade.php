...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="lg:col-span-2 bg-white rounded-xl border p-5">
        ...
        <div class="flex items-end gap-[4px] h-44">
            ...
            <div class="flex-1 flex flex-col items-center gap-1 group cursor-pointer" title="{{ $dailyLabels[$i] }}: TSh {{ number_format($rev) }}">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-0.5">
                        <p class="text-xs font-medium text-gray-900">{{ $m->method }}</p>
                        <p class="text-xs font-semibold text-gray-900">TSh {{ number_format($m->total) }}</p>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                        <div class="bg-emerald-500 h-1.5 rounded-full" style="width: {{ $stats['totalVolume'] > 0 ? ($m->total / $stats['totalVolume'] * 100) : 0 }}%"></div>
                    </div>
                    <p class="text-[10px] text-gray-400 mt-0.5">{{ $m->count }} transactions</p>
                </div>
            </div>
            @empty
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead><tr class="text-left text-xs text-gray-500 bg-gray-50/50">
                    ...
                    <th class="px-5 py-2.5 font-medium">Merchant</th>
                ...
                <tbody>
                    ...
                    <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-5 py-2.5 text-xs text-gray-700">{{ $tx->user?->business_name ?? $tx->user?->first_name ?? 'Unknown' }}</td>
                        <td class="px-5 py-2.5 text-xs font-semibold text-gray-900">TSh {{ number_format($tx->amount) }}</td>
                    ...
                    @empty
                    <tr><td colspan="4" class="px-5 py-8 text-center text-gray-400 text-xs">No transactions</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="bg-white rounded-xl border overflow-hidden">
        <div class="px-5 py-4 border-b flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-900">Top Merchants</h3>
        ...
        <div class="p-5 space-y-3">
            ...
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-xs">
                    {{ strtoupper(substr($merchant->first_name ?? 'U', 0, 1)) }}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900 truncate">{{ $merchant->business_name ?? $merchant->first_name . ' ' . $merchant->last_name }}</p>
                    <p class="text-xs text-gray-400">{{ $merchant->email }}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm font-bold text-gray-900">TSh {{ number_format($merchant->transactions_sum_amount ?? 0) }}</p>
                </div>
            </div>
            @empty
            <p class="text-sm text-gray-400 text-center py-4">No merchants yet</p>
            @endforelse
        </div>
    </div>
</div>
